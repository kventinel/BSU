...
import Laba12.editor.MySaxParser;
import Laba12.editor.MyTableModel;
...
public class MainWindow extends JFrame {
    ...
    private List<Laba12.data.Student> list;
    ...
    private MainWindow() {
        ...
        try {
            ...
            addExam.addActionListener(e -> {
                list.get(comboBox.getSelectedIndex()).getList().add(new Laba12.data.ExamPass("SomeName", "SomeExaminator", 1));
                updateVeryVery();
            });
            addTest.addActionListener(e -> {
                list.get(comboBox.getSelectedIndex()).getList().add(new Laba12.data.TestPass("SomeName", "SomeExaminator", false));
                updateVeryVery();
            });
            remExam.addActionListener(e -> {
                int idx = table.getSelectedRow();
                List<Laba12.data.AbstractPass> l = list.get(comboBox.getSelectedIndex()).getList();
                if (0 <= idx && idx < l.size()) l.remove(idx);
                updateVeryVery();
            });
            addStudent.addActionListener(e -> {
                Laba12.data.Student st = new Laba12.data.Student(textField.getText());
                list.add(st);
    ...
    private void readBinFile() throws IOException, ClassNotFoundException {
        ...
        list = (List<Laba12.data.Student>) in.readObject();
        updateInfo();
    }
    ...
    private void updateInfo() {
        ...
        for (Laba12.data.Student x : list) {
    ...
    private void initXML() throws ParserConfigurationException, SAXException {
        SchemaFactory schemaFactory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
        URL url = Class.class.getResource("/Laba12/xsd/Schema.xsd");
    ...
    private void readFile() throws IOException, SAXException {
        ...
        List<Laba12.data.Student> list = new ArrayList<>(size);
        for (int i = 0; i < size; i++) {
            ...
            List<Laba12.data.AbstractPass> passes = new ArrayList<>(esise);
            for (int j = 0; j < esise; j++) {
                ...
                if (type.compareTo("TestPass") == 0) {
                    ...
                    passes.add(new Laba12.data.TestPass(exname, examinator, passed));
                } else if (type.compareTo("ExamPass") == 0) {
                    String examinator = exam.getAttributes().getNamedItem("examinator").getTextContent();
                    String exname = exam.getTextContent();
                    int mark = Integer.parseInt(exam.getAttributes().getNamedItem("mark").getTextContent());
                    passes.add(new Laba12.data.ExamPass(exname, examinator, mark));
                }
            }
            list.add(new Laba12.data.Student(name, passes));
        }
        this.list = list;
        updateInfo();
    }
